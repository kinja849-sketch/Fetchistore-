...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-[100] bg-[#FBF9F8]/95 backdrop-blur-xl border-t border-[#E4E2E1] grid grid-cols-5 items-center min-h-[60px] py-1.5 px-1 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] md:hidden pointer-events-auto w-full max-w-full overflow-hidden">
      {navItems.map((item) => {
        const isActive =
          pathname === item.href ||
        ...
        return (
          <Link
            key={item.name}
            href={item.href}
            id={item.name === "Cart" ? "cart-fly-target" : undefined}
            className={`relative flex flex-col items-center justify-center py-0.5 px-0.5 rounded-2xl transition-all duration-200 w-full h-full text-center ${
              isActive
          ...
          >
            <div
              className={`relative flex items-center justify-center px-2.5 py-0.5 rounded-full transition-colors ${
            ...
            <span
              className={`text-[10px] sm:text-[11px] tracking-tight mt-0.5 whitespace-nowrap truncate max-w-full ${
                isActive ? "font-bold text-[#56642B]" : "font-semibold text-[#76786B]"
              }`}
            >
              {item.name}
            </span>
